class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        ...
        # so the points closest to the origin appear first

        # while we still have k closest points to origin to fulfill...
        while k > 0: 
            dist, x, y = heapq.heappop(minHeap) # pop the closest x and y coords to the origin
            res.append([x, y]) # add said coords to the results array
            k -= 1 # one fewer point left to collect
