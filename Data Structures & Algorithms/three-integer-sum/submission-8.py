class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i, a in enumerate(nums): # find index and value
            if a > 0: # since the array is sorted, no three positive numbers can sum to 0
                break

            if i > 0 and a == nums[i - 1]: # skip duplicate first elements to avoid duplicate triplets
                continue
            
            l, r = i + 1, len(nums) - 1 # skip first indice since we selected one already

            while l < r: # search until the pointers meet
