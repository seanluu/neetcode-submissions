class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        # 1) Base case -> when the path length geq len(nums)
        # 2) Choices -> either include nums[i] or skip nums[i] in the subset
        # 3) Constraints -> i must stay within the bounds of the array
        # 4) Backtracking step -> pop the last number added
